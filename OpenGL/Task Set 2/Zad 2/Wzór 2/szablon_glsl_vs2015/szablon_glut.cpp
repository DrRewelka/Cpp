...
void renderScene()
{
	...
	mvMatrix *= glm::translate(glm::vec3(-2.6f, 2.7f, 0.0f));
	mvMatrix *= glm::scale(glm::vec3(0.65f));
	...
	for (int j = 0; j <= 4; j++) //petla rysujaca kolejne linie kwadratow
	{
		...
		mvMatrix *= glm::translate(glm::vec3(-8.0f, -2.0f, 0.0f));
		glUniformMatrix4fv(mvMatrixLoc, 1, GL_FALSE, glm::value_ptr(mvMatrix));
	...
	rotationAngles.z = 180.0f; //zmiana kata obrotu
	
	//wyrysowanie pierwszego trojkata - lewy gorny rog, gorna krawedz (po transformacjach)
	glBindVertexArray( vao[1] );
	mvMatrix *= glm::translate(glm::vec3(-3.5f, 5.5f, 0.0f));
	...
	//rysowanie pierwszej linii trojkatow, gorny i dolny trojkat danego kwadratu
	for (int j = 0; j <= 4; j++) //petla rysujaca kolejne linie trojkatow
	{
		for (int i = 0; i <= 8; i++) //petla rysujaca trojkaty w linii
		{
			if (i % 2 == 0) //rysowanie naprzemian dolnego i gornego trojkata dla kwadratu
			{
				mvMatrix *= glm::rotate(glm::radians(rotationAngles.z), glm::vec3(0.0f, 0.0f, 1.0f));
				mvMatrix *= glm::translate(glm::vec3(0.0f, -1.5f, 0.0f));
				glUniformMatrix4fv(mvMatrixLoc, 1, GL_FALSE, glm::value_ptr(mvMatrix));
			...
			{
				mvMatrix *= glm::translate(glm::vec3(2.0f, 1.5f, 0.0f));
				mvMatrix *= glm::rotate(glm::radians(-rotationAngles.z), glm::vec3(0.0f, 0.0f, 1.0f));
				glUniformMatrix4fv(mvMatrixLoc, 1, GL_FALSE, glm::value_ptr(mvMatrix));
				glDrawArrays(GL_TRIANGLES, 0, 3);
				triangleCounter++;
		...
		mvMatrix *= glm::translate(glm::vec3(-8.0f, -0.5f, 0.0f));
		mvMatrix *= glm::rotate(glm::radians(-rotationAngles.z), glm::vec3(0.0f, 0.0f, 1.0f));
		glUniformMatrix4fv(mvMatrixLoc, 1, GL_FALSE, glm::value_ptr(mvMatrix));
	...
	//przejscie do pierwszego trojkata przy lewym gornym kwadracie, na jego lewej krawedzi
	mvMatrix *= glm::rotate(glm::radians(rotationAngles.z), glm::vec3(0.0f, 0.0f, 1.0f));
	rotationAngles.z = 90.0f; //zmiana kata obrotu
	mvMatrix *= glm::translate(glm::vec3(-0.75f, 9.25f, 0.0f));
	mvMatrix *= glm::rotate(glm::radians(-rotationAngles.z), glm::vec3(0.0f, 0.0f, 1.0f));
	glUniformMatrix4fv(mvMatrixLoc, 1, GL_FALSE, glm::value_ptr(mvMatrix));
	glDrawArrays(GL_TRIANGLES, 0, 3); //rysowanie
	triangleCounter++;

	//rysowanie drugiej linii trojkatow, czyli lewy i prawy trojkat przy danym kwadracie
	for (int j = 0; j <= 4; j++) //petla, ktora rysuje kolejne linie
	{
		for (int i = 0; i <= 8; i++) //petla rysujaca kolejne trojkaty w jednej linii
		{
			...
			{
				mvMatrix *= glm::rotate(glm::radians(rotationAngles.z), glm::vec3(0.0f, 0.0f, 1.0f));
				mvMatrix *= glm::translate(glm::vec3(1.5f, 0.0f, 0.0f));
				mvMatrix *= glm::rotate(glm::radians(rotationAngles.z), glm::vec3(0.0f, 0.0f, 1.0f));
				glUniformMatrix4fv(mvMatrixLoc, 1, GL_FALSE, glm::value_ptr(mvMatrix));
				glDrawArrays(GL_TRIANGLES, 0, 3);
				triangleCounter++;
			...
			{
				mvMatrix *= glm::rotate(glm::radians(-rotationAngles.z), glm::vec3(0.0f, 0.0f, 1.0f));
				mvMatrix *= glm::translate(glm::vec3(0.5f, 0.0f, 0.0f));
				mvMatrix *= glm::rotate(glm::radians(-rotationAngles.z), glm::vec3(0.0f, 0.0f, 1.0f));
				glUniformMatrix4fv(mvMatrixLoc, 1, GL_FALSE, glm::value_ptr(mvMatrix));
				glDrawArrays(GL_TRIANGLES, 0, 3);
				triangleCounter++;
		...
		mvMatrix *= glm::rotate(glm::radians(-rotationAngles.z), glm::vec3(0.0f, 0.0f, 1.0f));
		mvMatrix *= glm::translate(glm::vec3(-9.5f, -2.0f, 0.0f));
		mvMatrix *= glm::rotate(glm::radians(-rotationAngles.z), glm::vec3(0.0f, 0.0f, 1.0f));
		glUniformMatrix4fv(mvMatrixLoc, 1, GL_FALSE, glm::value_ptr(mvMatrix));
