...
void renderScene()
{
	...
	//rysowanie cylindra reprezentowanego przez vao
...
/*------------------------------------------------------------------------------------------
** funkcja tworzaca VAO i VBO z cylindrem
**------------------------------------------------------------------------------------------*/
void setupBuffers()
{
	...
	glBindVertexArray(vao); //powiazanie vao, reprezentuje cylinder
	...
	float zMax = 1.0f; //wysokosc cylindra
	float radius = 0.5f; //promien obiektu
	float x, y, z, u, v; //wspolrzedne i zmienne u, v - kolejne wierzcholki
	float theta; //kat wypelnienia

	//obliczenia wszystkich wartosci do wyrysowania cylindra
	for (int i = 0; i < verticalVertices; i++) //petla do kolejnych poziomow wierzcholkow
	{
		...
		{											 //na jednym poziomie
			...
			x = radius * cos(theta);				 //wspolrzedna x wierzcholka
			y = radius * sin(theta);				 //wspolrzedna y wierzcholka
			z = v * zMax;							 //wspolrzedna z wierzcholka
		...
		if (i == 0 || i == (verticalVertices - 1)) //gorna i dolna podstawa cylindra
		{
			for (int j = 0; j < horizontalVertices; j++)
			{
				triangles.push_back(i * horizontalVertices);
				triangles.push_back(i * horizontalVertices + j);
				triangles.push_back(i * horizontalVertices + j + 1);
			}
		}
