...
void renderScene()
{
	...
	//rysowanie stozka reprezentowanego przez vao
...
/*------------------------------------------------------------------------------------------
** funkcja tworzaca VAO i VBO ze stozkiem
**------------------------------------------------------------------------------------------*/
void setupBuffers()
{
	...
	glBindVertexArray(vao); //powiazanie vao, reprezentuje stozek
	...
	float height = 1.0f; //wysokosc stozka
	...
	for (int i = 0; i < verticalVertices; i++) //petla do kolejnych poziomow wierzcholkow
	{
		...
		{											 //na jednym poziomie
			...
			x = radius * (1 - v) * cos(theta);		 //wspolrzedne x wierzcholka
			y = radius * (1 - v) * sin(theta);		 //wspolrzedne y wierzcholka
			z = v * height;							 //wspolrzedne z wierzcholka
