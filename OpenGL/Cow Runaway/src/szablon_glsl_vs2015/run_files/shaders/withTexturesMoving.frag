...
void main()
{
	...
	vec3 diffuse = lightDiffuse * vec3(texture(texture_diffuse1, fTexCoords)) * max( dot( lightDir, norm ), 0.0 );
	...
	if( dot( lightDir, viewDir ) > 0.0 )
	{
		...
		specular = pow( max( 0.0, dot( viewDir, refl ) ), materialShininess ) * vec3(texture(texture_specular1, fTexCoords)) * lightSpecular;
	}
