...
enum Shader { NOTEXTURES, WITHTEXTURES };
...
Shader shader = NOTEXTURES; // aktualny program cieniowania
		...
		if (centerOfMass != glm::vec3(0.0f, 0.0f, 0.0f)) //Przesuniecie, jezeli obiekt nie byl na srodku
		{
			for (int i = 0; i < mesh->mNumVertices; i++)
			{
				...
				if (mesh->mTextureCoords[0]) //Nie wiadomo, czy mesh ma tekstury
				{
					glm::vec2 vec;
					//Wierzcholek moze miec do 8 wspolrzednych tekstury, ale przyjmujemy, ze uzywamy tylko x i y
					vec.x = mesh->mTextureCoords[0][i].x - centerOfMass.x;
					vec.y = mesh->mTextureCoords[0][i].y - centerOfMass.y;
					vertex.TexCoords = vec;
				}
				else
					vertex.TexCoords = glm::vec2(0.0f, 0.0f);
	...
	std::vector<Texture> loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
	{
		...
		for (GLuint i = 0; i <= mat->GetTextureCount(type); i++)
		{
			...
			if (!skip)
			{
				...
				Texture texture;
				texture.id = TextureFromFile((typeName + ".png").c_str(), this->directory); //Wczytanie tekstury, 
				//mozna wpisac dowolna nazwe zamiast pierwszego parametru, tekstura musi byc w folderze z modelem,
				//w tym wypadku jest to folder /modele
...
void initGL()
{
	...
	myModel = new Model("modele/dino.obj");
...
void setupShaders()
{
	if (!setupShaders("shaders/noTextures.vert", "shaders/noTextures.frag", shaderProgram[NOTEXTURES]))
		exit(3);
	if (!setupShaders("shaders/withTextures.vert", "shaders/withTextures.frag", shaderProgram[WITHTEXTURES]))
		exit(3);
