//Model po lewej - projekcja planarna, sterowanie: rotacja - i, j, k, l, y, u; skalowanie - o, p
//Model po prawej - projekcja sferyczna, sterowanie: rotacja - w, a, s, d, q, z; skalowanie - r, f
...
enum Shader { PLANEPROJECTION, SPHEREPROJECTION };
...
glm::vec3 rot1 = glm::vec3(0.0, 0.0, 0.0); // katy rotacji wokol poszczegolnych osi dla modelu 1
glm::vec3 scl1 = glm::vec3(1.0, 1.0, 1.0); //Skalowanie dla modelu 1
glm::vec3 rot2 = glm::vec3(0.0, 0.0, 0.0); // katy rotacji wokol poszczegolnych osi dla modelu 2
glm::vec3 scl2 = glm::vec3(1.0, 1.0, 1.0); //Skalowanie dla modelu 2
...
private:
	...
	Mesh processMesh(aiMesh* mesh, const aiScene* scene)
	{
		...
		//Sprawdzenie, czy srodek masy jest na srodku ekranu, jezeli nie to przesuniecie calego modelu
		glm::vec3 sumOfVerts;
		glm::vec3 centerOfMass;

		for (int i = 0; i < mesh->mNumVertices; i++)
		{
			sumOfVerts.x += mesh->mVertices[i].x;
			sumOfVerts.y += mesh->mVertices[i].y;
			sumOfVerts.z += mesh->mVertices[i].z;
		}

		centerOfMass.x = sumOfVerts.x / (mesh->mNumVertices);
		centerOfMass.y = sumOfVerts.y / (mesh->mNumVertices);
		centerOfMass.z = sumOfVerts.z / (mesh->mNumVertices);

		if (centerOfMass != glm::vec3(0.0f, 0.0f, 0.0f)) //Przesuniecie, jezeli obiekt nie byl na srodku
		{
			for (int i = 0; i < mesh->mNumVertices; i++)
			{
				Vertex vertex;
				...
				glm::vec3 vector; //Wektor pomocniczy, ASSIMP korzysta z wlasnego wektora, ktorego nie mozna
								  //bezposrednio przekonwertowac do glm::vec3
								  //Pozycje wierzcholkow
				vector.x = mesh->mVertices[i].x - centerOfMass.x;
				vector.y = mesh->mVertices[i].y - centerOfMass.y;
				vector.z = mesh->mVertices[i].z - centerOfMass.z;
				vertex.Position = vector;
				//Normale
				vector.x = mesh->mNormals[i].x - centerOfMass.x;
				vector.y = mesh->mNormals[i].y - centerOfMass.y;
				vector.z = mesh->mNormals[i].z - centerOfMass.z;
				vertex.Normal = vector;

				vertices[i] = vertex; //Gotowy wierzcholek wrzucamy do wczesniej zadeklarowanego kontenera
			}
		}
	...
	std::vector<Texture> loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
	{
		...
		{
			aiString str;
			std::string name = "texture_diffuse.png"; //Nazwa tekstury, plik z tekstura musi byc w folderze z modelem
			...
			if (!skip)
			{
				...
				Texture texture;
				texture.id = TextureFromFile((name).c_str(), this->directory); //Wczytanie tekstury
...
Model *firstModel; //Deklaracja pierwszego modelu
Model *secondModel; //Deklaracja drugiego modelu
...
void initGL()
{
	...
	firstModel = new Model("modele/teapot.obj");
	secondModel = new Model("modele/duckyt.obj");
...
void renderScene()
{
	...
	//Pierwszy model
	glUseProgram(shaderProgram[SPHEREPROJECTION]);
	glUniformMatrix4fv(projMatrixLoc, 1, GL_FALSE, glm::value_ptr(projMatrix));

	lightPos = mvMatrix * lightPosition; // przeliczenie polozenia swiatla do ukladu oka
	...
	mvMatrix *= glm::translate(glm::vec3(2.0f, 0.0f, 0.0f));
	//Obrot
	mvMatrix *= glm::rotate(glm::radians(rot1.z), glm::vec3(0.0f, 0.0f, 1.0f));
	mvMatrix *= glm::rotate(glm::radians(rot1.y), glm::vec3(0.0f, 1.0f, 0.0f));
	mvMatrix *= glm::rotate(glm::radians(rot1.x), glm::vec3(1.0f, 0.0f, 0.0f));

	//Skalowanie
	mvMatrix *= glm::scale(glm::vec3(scl1.x, scl1.y, scl1.z));

	glUniformMatrix4fv(mvMatrixLoc, 1, GL_FALSE, glm::value_ptr(mvMatrix));

	normalMat = glm::inverseTranspose(glm::mat3(mvMatrix));
	glUniformMatrix3fv(normalMatrixLoc, 1, GL_FALSE, glm::value_ptr(normalMat));
	
	firstModel->Draw(SPHEREPROJECTION);
	...
	//Drugi model
	glUseProgram(shaderProgram[PLANEPROJECTION]);
	glUniformMatrix4fv(projMatrixLoc, 1, GL_FALSE, glm::value_ptr(projMatrix));
	...
	mvMatrix *= glm::translate(glm::vec3(-2.0f, 0.0f, 0.0f));
	//Obrot
	mvMatrix *= glm::rotate(glm::radians(rot2.z), glm::vec3(0.0f, 0.0f, 1.0f));
	mvMatrix *= glm::rotate(glm::radians(rot2.y), glm::vec3(0.0f, 1.0f, 0.0f));
	mvMatrix *= glm::rotate(glm::radians(rot2.x), glm::vec3(1.0f, 0.0f, 0.0f));

	//Skalowanie
	mvMatrix *= glm::scale(glm::vec3(scl2.x, scl2.y, scl2.z));
	...
	secondModel->Draw(PLANEPROJECTION);
...
void setupShaders()
{
	if (!setupShaders("shaders/planeProjection.vert", "shaders/planeProjection.frag", shaderProgram[PLANEPROJECTION]))
		exit(3);
	if (!setupShaders("shaders/sphereProjection.vert", "shaders/sphereProjection.frag", shaderProgram[SPHEREPROJECTION]))
		exit(3);
