...
void main()
{
	...
	if( dot( lightDir, viewDir ) > 0.0 )
	{
		...
		float expValue = (1 - pow(dot(norm, halved), 2))/pow(dot(norm, halved), 2); //Zmienna pomocnicza
		specular = lightDiffuse * materialDiffuse * exp(-2 * expValue); //Swiatlo w modelu Warda z cieniowaniem Phonga
	}
