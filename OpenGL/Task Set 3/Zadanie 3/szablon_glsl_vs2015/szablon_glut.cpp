...
enum Shader { WARDPHONG, WARDGOURAUD };
...
Shader shader = WARDPHONG; // aktualny program cieniowania
...
void setupShaders()
{
	if (!setupShaders("shaders/wardPhong.vert", "shaders/wardPhong.frag", shaderProgram[WARDPHONG]))
		exit(3);
	if (!setupShaders("shaders/wardGouraud.vert", "shaders/wardGouraud.frag", shaderProgram[WARDGOURAUD]))
		exit(3);
